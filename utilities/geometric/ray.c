...
ray specular_ray(normal surface_normal, vector surface_point, ray incoming_ray, double angle_spread) {
  normal specular_direction = to_normal(sub_vector(incoming_ray.direction,
                                                   multiply(surface_normal,
                                                            2.0 * dot(incoming_ray.direction, surface_normal))));
...
ray refract_ray(normal surface_normal,
                ...
                double n) {
  normal
      refracted_direction = to_normal(sub_vector(multiply(incoming_ray.direction, n), multiply(surface_normal, cosT)));
  if (angle_spread < ERROR) {
    return new_ray(
        surface_point,
        refracted_direction
