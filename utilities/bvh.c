...
void distribute_bvh(bvh_tree *root) {
  ...
  while (stack.length > 0) {
    ...
    for (size_t m = 0; m < 3; m++) {
      switch (ejes[m]) {
        case X:qsort(array, current->num_of_objects, sizeof(object *), compare_by_x);

          break;
        case Y:qsort(array, current->num_of_objects, sizeof(object *), compare_by_y);
          break;
        case Z:qsort(array, current->num_of_objects, sizeof(object *), compare_by_z);
          break;
        default:break;
      }
