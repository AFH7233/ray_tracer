  ...
  for (size_t i = data->start_w; i < data->end_w; i++) {
    ...
    for (size_t j = data->start_h; j < data->end_h; j++) {
      ...
      for (size_t k = 0; k < data->rays_per_pixel; k++) {
        ...
        ray_color =
            add_color(ray_color, render_pixel(pixel_ray, data->tree, data->bounces, data->ambient_color, stack));
      }
  ...
  if (bounces > 0) {
    ...
    if (hitted_object.is_hit && !hitted_object.material.is_absorbing) {
      ...
      normal corrected_normal =
          dot(surface_normal, pixel_ray.direction) < 0.0 ? surface_normal : multiply(surface_normal, -1.0);
      ...
      if (hitted_object.material.is_dielectric) {
        ...
        if (cosT2 < 0.0) {
          generated_pixel_ray =
              specular_ray(corrected_normal, surface_point, pixel_ray, hitted_object.material.angle_spread_reflect);
        } else {
          if (stack.ids[stack.length - 1] == hitted_object.id) {
            stack.length--;
          } else {
          ...
          generated_pixel_ray = refract_ray(corrected_normal,
                                            surface_point,
                                            pixel_ray,
                                            hitted_object.material.angle_spread_reflect,
                                            cosT,
                                            n);
        }
      } else {
